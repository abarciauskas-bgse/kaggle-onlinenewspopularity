# K MEANS CLUSTERING
# IN THIS FILE
# We perform a logistic regression on the newspop dataset using 3 clusters
  ...
  for(iter in 1:iterations) {
    ...
    for(k in 1:clusters) {
      ...
      for(i in 1:5) {
        ...
        errors[,i] <- predict(model, newdata = newspop.test[newspop.test$cluster == k,])
      }
...
write.csv(sample, 'finallogistkmeans.csv', row.names = FALSE)